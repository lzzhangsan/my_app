...
''');
  ...
  Future<String> exportAllData() async {
    try {
      ...
      final Map<String, List<Map<String, dynamic>>> tableData = {
        ...
        'directory_settings': await db.query('directory_settings'),
        'document_settings': await db.query('document_settings'),
      };

      // 特殊处理音频框数据
      List<Map<String, dynamic>> audioBoxes = await db.query('audio_boxes');
      print('找到 ${audioBoxes.length} 个音频框');
      List<Map<String, dynamic>> audioBoxesToExport = [];
      for (var audioBox in audioBoxes) {
        Map<String, dynamic> audioBoxCopy = Map<String, dynamic>.from(audioBox);
        String audioPath = audioBox['audioPath'];
        print('处理音频框 - ID: ${audioBox['id']}, 原始路径: $audioPath');
        if (audioPath.isNotEmpty) {
          String fileName = p.basename(audioPath);
          audioBoxCopy['audioFileName'] = fileName;
          print('设置音频文件名: $fileName');
        }
        audioBoxesToExport.add(audioBoxCopy);
      }
      ...
      if (await documentsDir.exists()) {
        final Directory targetDir = Directory('$tempDirPath/documents');
        await targetDir.create();
        await for (var entity in documentsDir.list(recursive: true)) {
          if (entity is File) {
            final String relativePath = entity.path.substring(documentsPath.length);
            final String targetPath = '$tempDirPath/documents$relativePath';
      ...
      final Directory backgroundImagesDir = Directory(backgroundImagesPath);
      if (await backgroundImagesDir.exists()) {
        final Directory targetDir = Directory('$tempDirPath/background_images');
        await targetDir.create();
        await for (var entity in backgroundImagesDir.list(recursive: true)) {
          if (entity is File) {
            final String relativePath = entity.path.substring(backgroundImagesPath.length);
            final String targetPath = '$tempDirPath/background_images$relativePath';
            await Directory(p.dirname(targetPath)).create(recursive: true);
            await entity.copy(targetPath);
          }
        }
      ...
      // 导出媒体文件
      final String imagesPath = '${appDocDir.path}/images';
      final Directory imagesDir = Directory(imagesPath);
      if (await imagesDir.exists()) {
        final Directory targetDir = Directory('$tempDirPath/images');
        await targetDir.create();
        await for (var entity in imagesDir.list(recursive: true)) {
          if (entity is File) {
            final String relativePath = entity.path.substring(imagesPath.length);
            final String targetPath = '$tempDirPath/images$relativePath';
            await Directory(p.dirname(targetPath)).create(recursive: true);
            await entity.copy(targetPath);
          }
        }
      ...
      // 处理音频文件
      final String audiosPath = '${appDocDir.path}/audios';
      final Directory audiosDir = Directory(audiosPath);
      print('开始处理音频文件...');
      
      // 创建临时音频目录
      final Directory targetAudioDir = Directory('$tempDirPath/audios');
      await targetAudioDir.create(recursive: true);
      print('创建临时音频目录: ${targetAudioDir.path}');
      ...
      for (var audioBox in audioBoxes) {
        ...
        if (audioPath.isNotEmpty) {
          File audioFile = File(audioPath);
          String fileName = p.basename(audioPath);
          String targetPath = p.join(targetAudioDir.path, fileName);
          
          print('处理音频文件: $audioPath');
          if (await audioFile.exists()) {
            try {
              // 复制文件并确保写入完成
              await audioFile.copy(targetPath);
              File targetFile = File(targetPath);
              await targetFile.writeAsBytes(await audioFile.readAsBytes(), flush: true);
              
              if (await targetFile.exists()) {
                final int fileSize = await targetFile.length();
                print('成功复制音频文件: $targetPath (大小: $fileSize 字节)');
                successCount++;
              } else {
                print('警告：音频文件复制后不存在: $targetPath');
              }
            } catch (e) {
              print('复制音频文件时出错: $e');
            }
          } else {
            print('警告：源音频文件不存在: $audioPath');
          }
        }
      }
      
      print('音频文件处理完成，成功复制 $successCount/${audioBoxes.length} 个文件');
  ...
  Future<void> importAllData(String zipPath) async {
    try {
      ...
      print('成功解析数据文件，包含以下表: ${tableData.keys.join(', ')}');
      ...
      await db.transaction((txn) async {
        ...
        for (var entry in tableData.entries) {
          ...
          if (tableName == 'audio_boxes') {
            print('开始处理音频框数据...');
            for (var row in rows) {
              Map<String, dynamic> audioBox = Map<String, dynamic>.from(row);
              String? audioFileName = audioBox['audioFileName'];
              print('处理音频框 - ID: ${audioBox['id']}, 文件名: $audioFileName');
              
              if (audioFileName != null) {
                String audiosDirPath = p.join(appDocDir.path, 'audios');
                String audioPathToSave = p.join(audiosDirPath, audioFileName);
                audioBox['audioPath'] = audioPathToSave;
                print('设置新的音频路径: $audioPathToSave');
                
                // 验证文件是否存在
                if (await File(audioPathToSave).exists()) {
                  print('音频文件已存在: $audioPathToSave');
                } else {
                  print('警告：音频文件不存在: $audioPathToSave');
                }
              }
              audioBox.remove('audioFileName');
            ...
            for (var row in rows) {
              ...
              if (backgroundImagePath != null && backgroundImagePath.isNotEmpty) {
                String fileName = p.basename(backgroundImagePath);
                String newPath = p.join(appDocDir.path, 'background_images', fileName);
                settings['background_image_path'] = newPath;
                print('更新目录背景图片路径: $newPath');
              }
      ...
      final Directory tempDocsDir = Directory('$tempDirPath/documents');
      if (await tempDocsDir.exists()) {
        await tempDocsDir.rename(documentsPath);
      }
      ...
      final Directory backgroundImagesDir = Directory(backgroundImagesPath);
      if (await backgroundImagesDir.exists()) {
        await backgroundImagesDir.delete(recursive: true);
      }
      final Directory tempBgDir = Directory('$tempDirPath/background_images');
      if (await tempBgDir.exists()) {
        await tempBgDir.rename(backgroundImagesPath);
      }
      ...
      final Directory tempImagesDir = Directory('$tempDirPath/images');
      if (await tempImagesDir.exists()) {
        await tempImagesDir.rename(imagesPath);
      }
      ...
      await Directory(audiosPath).create(recursive: true);

      final Directory tempAudiosDir = Directory('$tempDirPath/audios');
      if (await tempAudiosDir.exists()) {
        print('开始复制音频文件...');
        await for (var entity in tempAudiosDir.list(recursive: true)) {
          if (entity is File) {
            final String fileName = p.basename(entity.path);
            final String targetPath = p.join(audiosPath, fileName);
            print('复制音频文件: ${entity.path} -> $targetPath');
            await entity.copy(targetPath);
            // 确保文件写入完成
            final File targetFile = File(targetPath);
            await targetFile.writeAsBytes(await entity.readAsBytes(), flush: true);
            print('音频文件复制完成: $targetPath');
          }
        }
      }
      ...
      for (var file in archive) {
        if (file.isFile && file.name.startsWith('audios/')) {
          ...
          final String targetPath = p.join(audiosPath, fileName);
          print('处理音频文件: ${file.name} -> $targetPath');
          
          // 直接从ZIP内容写入文件
          final File audioFile = File(targetPath);
          await audioFile.writeAsBytes(file.content as List<int>, flush: true);
          print('音频文件写入完成: $targetPath');
          
          // 验证文件是否成功写入
          if (await audioFile.exists()) {
            final int fileSize = await audioFile.length();
            print('音频文件已写入，大小: $fileSize 字节');
          } else {
            print('警告：音频文件写入后不存在: $targetPath');
          }
        }
      }
