...
class _CoverPageState extends State<CoverPage> with WidgetsBindingObserver {
  ...
  Future<void> _persistCoverState() async {
    if (kIsWeb) return;
    try {
  ...
  Future<void> _removeBackgroundImage() async {
    ...
    if (shouldDelete) {
      try {
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('删除背景设置时出错，请重试。')));
      }
    }
  }
  ...
  Future<void> _removeBackgroundImageOnly() async {
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('删除背景图片时出错，请重试。')));
    }
  }
  ...
  Future<void> _saveBackgroundImage(String imagePath) async {
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('保存背景图片失败，请重试。')));
    }
  }
  ...
  Future<void> _loadContent() async {
    try {
      List<Map<String, dynamic>> textBoxes = await getService<DatabaseService>()
          .getTextBoxesByDocument(coverDocumentName);
      setState(() {
        _textBoxes =
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('加载内容时出错，请重试。')));
    }
  }
  ...
  Future<void> _saveContent() async {
    try {
      ...
      await getService<DatabaseService>().saveTextBoxes(
        _textBoxes,
        coverDocumentName,
      );
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('保存内容时出错，请重试。')));
    }
  }
  ...
  Future<void> _setBackgroundColor(Color color) async {
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('设置背景颜色失败，请重试')));
    }
  }
  ...
  Future<void> _clearAllContent() async {
    ...
    if (shouldClear) {
      try {
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('清空封面页时出错，请重试。')));
      }
    }
  }
  ...
  Future<void> _cleanImportTempFiles() async {
    try {
      ...
      if (result['success'] == true) {
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('清理失败: ${result['error']}')));
      }
    } catch (e) {
      if (mounted) {
        Navigator.of(context).pop(); // 关闭进度对话框
